...
from pipelines.functions import (
    drop_too_much,
    searching_possible_transformable_variables,
    is_inDay,
)
...
NY_copy["crash_date"] = pd.to_datetime(
    NY_copy["crash_date"], infer_datetime_format=True
)
...
to_drop = NY_copy[
    (NY_copy["latitude"] <= 1) & (NY_copy["longitude"] >= -1)
    | (NY_copy["longitude"] <= 77) & (NY_copy["on_street_name"].isnull())
].index
NY_copy.drop(to_drop, inplace=True)

# transforming borough values into binaries
test = pd.get_dummies(NY_copy["borough"], prefix="borough_is")
...
try:
    vehicles = [
        *map(str.lower, list(NY_copy["vehicle_type_code1"].value_counts().index))
    ]
except KeyError:
    vehicles = [
        *map(str.lower, list(NY_copy["vehicle_type_code_1"].value_counts().index))
    ]
...
final.drop(
    ["vehicle_type_code1", "vehicle_type_code2", "collision_id"], inplace=True, axis=1
)
